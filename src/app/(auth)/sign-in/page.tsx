...
const page = () => {
  ...
  const [isSubmitting, setIsSubmitting] = useState(false)
     const debouncedUser  = useDebounceValue(username,300)
     ...
     useEffect(() => {
      const checUsernameUnique = async () => {
        if(debouncedUser){
          ...
          setUsernameMessage('')
          try {
              const response = await axios.get(`/api/username-unique?username=${debouncedUser}`)
              console.log("response of axios is that", response)
              setUsernameMessage(response.data.message)
          } catch (error) {
            const axioserror = error as AxiosError<ApiResponse>
            setUsernameMessage(
              axioserror.response?.data.message?? "Error chec username"
            )
          }
          finally{
     ...
     checUsernameUnique()  }, [debouncedUser])
